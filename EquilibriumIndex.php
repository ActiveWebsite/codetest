...
function getEquilibriums($arr) {
	...
	if (is_array($arr)){
		...
		// We need to iterate over all elements in the array $arr only once
		// to get, on the same iteration, the sum of all elements on the right and sum of elements on the left of the 
		// current index
		for ($index=0; $index < $length; $index++) { 

			
			// Add the value of the previous array element, if exists, to the variable that holds the 
			// sum of all array elements on that are on the left of the current index.
			// Make sure the previous index is a valid index, this means bigger or equal than 0.
			if ($index-1>=0){
				$left_acum 	+= $arr[$index-1];
			}

			// We start knowing the value of adding all the elements on the array ($arr), so if we start
			// on the element on the left (index 0) and we substract its value from the sum of all array elements , 
			// we are going to get a value that represents the sum of all elements on the array minus the value of  
			// the element on index 0. If we store that last value and we keep substracting each array values, one by one,from left to right,on each iteration we get the sum of all elements to the right of the current index . 
